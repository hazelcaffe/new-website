---
eyebrow: "meow :3c"
title: "Welcome to my website!"
---

## Hobbies
I wish I could draw :(
- Frontend & Backend Development
- IT & System Administration
- Cybersecurity
- JS & Malware Reverse Engineering
- Linux (NixOS <3)

## Programming Languages
I don't know many...
- TypeScript & JavaScript (I ♥ TYPESCRIPT)
- HTML & CSS
- Bash (I don't know Powershell, nor do I want to learn)
- I *used* to know Python...

## Video Games
I don't play many...
- Geometry Dash
- Project Sekai / Hatsune Miku: Colorful Stage!
- osu!mania (4k)
- Quaver

I'm also a [speedrun verifier for Hello Neighbor](https://speedrun.com/hn1)

## Music
I ***love*** music.
- I LOVE LEXYCAT
- Vocaloid/Synthetic music
    - My favorite artists/bands are DECO*27, SAWTOWNE, FLAVOR FOLEY, Jamie Paige, etc etc. 
- I also like some other artists like Yeat, Laufey, and Jane Remover

I use Apple Music and that is the only good streaming provider.

## Devices
I have a few :3

- M5 Macbook Pro
    - Storage: 512GB
    - RAM: 16GB
- Used Alienware PC
    - Storage: 2TB NVMe + 2TB SSD (Running NixOS + Windows 11)
    - RAM: 32GB DDR4
    - CPU: Ryzen 7 5800X
    - GPU: RTX 3070 LHR
    - My monitor is some 2K 180HZ Acer monitor
    - My mouse is a Logitech G502 🔥🔥
- Used iPhone 15 Pro
    - Storage: 128GB
    - RAM: 8GB
- My homelab is some used Dell workstation
    - 256GB SSD + 512GB NVMe for Proxmox (768GB total)
    - Storage: Two 2TB HDDs, and two 1TB HDDs (4TB total)
    - CPU: Intel Xeon E5-2687W v4 (3GHz, 12c/24t)
    - RAM: 48GB DDR4 ECC RDIMM
    - GPU: Nvidia Quadro K620